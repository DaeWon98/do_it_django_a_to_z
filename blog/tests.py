...
class TestView(TestCase):
    def setUp(self):
        self.client = Client()
        self.user_trump = User.objects.create_user(username='trump', password='korea2002')
        self.user_obama = User.objects.create_user(username='obama', password='korea2002')
    ...
    def test_create_post(self):
        ...
        self.client.login(username='trump', password='korea2002')
        response = self.client.get('/blog/create_post/')
        self.assertNotEqual(response.status_code, 200)

        # staff인 turmp가 로그인을 한다.
        self.client.login(username='obama', password='korea2002')
        response = self.client.get('/blog/create_post/')
        self.assertEqual(response.status_code, 200)
        soup = BeautifulSoup(response.content, 'html.parser')
