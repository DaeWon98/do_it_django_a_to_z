...
class TestView(TestCase):
    def setUp(self):
        self.client = Client()
        self.user_camry = User.objects.create_user(username='camry', password='korea2002')
        self.user_toyota = User.objects.create_user(username='toyota', password='korea2002')
        self.user_camry.is_staff = True
        self.user_camry.save()
    ...
    def test_create_post(self):
        ...
        # Staff가 아닌 Camry가 로그인을 한다.
        self.client.login(username='toyota', password='korea2002')
        response = self.client.get('/blog/create_post/')
        self.assertNotEqual(response.status_code, 200)

        # staff인 camry가 로그인을 한다.
        self.client.login(username='camry', password='korea2002')
        response = self.client.get('/blog/create_post/')
        self.assertEqual(response.status_code, 200)
        soup = BeautifulSoup(response.content, 'html.parser')
        ...
        self.assertEqual(last_post.author.username, 'camry')
